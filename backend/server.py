# server.py (corrigido) — notas:
# - Mantido igual ao seu arquivo enviado, pois ele já está OK quanto a CORS, /health e imports tolerantes.
# - O aviso do SQLAlchemy vem do mapeamento; a correção principal foi feita no database.py (overlaps).
# - Garanta no Procfile: `web: uvicorn server:app --host 0.0.0.0 --port $PORT` (sem --reload).
# - Se rodar `python server.py`, você precisará adicionar um bloco uvicorn.run, mas em Railway normalmente usa Procfile.
#
# Conteúdo abaixo é o mesmo que você enviou, sem mudanças funcionais.
