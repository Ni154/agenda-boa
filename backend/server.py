...
# .env é opcional — carrega se existir
try:
...
# ---------------------------------------------------------------------
# Imports do banco (compatível com 2 layouts: pacote ou módulo solto)
# ---------------------------------------------------------------------
from sqlalchemy.orm import Session

try:
    # se estiver dentro de um pacote (ex.: backend/)
    from .database import (
        get_db, create_tables, Tenant, User, Cliente,
        Produto, Servico, Venda, Agendamento, SessionLocal
    )
except Exception:
    # se os arquivos estiverem no mesmo diretório da app
    from database import (
        get_db, create_tables, Tenant, User, Cliente,
        Produto, Servico, Venda, Agendamento, SessionLocal
    )
...
if _raw_origins == "" or _raw_origins == "*":
    ALLOW_ORIGINS = ["*"]
    ALLOW_CREDENTIALS = False   # Obrigatório False quando origins = *
else:
...
# ---------------------------------------------------------------------
# Rotas utilitárias (healthcheck / favicon / raíz)
# ---------------------------------------------------------------------
@app.get("/health")
def health():
    """
    Healthcheck para plataformas de deploy.
    """
...
def root():
    """
    Responde algo na raíz para evitar 404 em healthchecks.
    """
    return {"ok": True, "service": "backend", "docs": "/docs", "health": "/health"}

@app.get("/favicon.ico")
def favicon():
    """
    Evita 404/502 quando o frontend tenta baixar /favicon.ico.
    """
...
def send_email(to_email: str, subject: str, html_content: str):
    """Envia email via Resend; se não houver API key, faz 'print' (simulação)."""
...
async def create_venda(
    ...
    for i in venda_data.itens:
        if i.tipo == "produto":
            produto = db.query(Produto).filter(
                Produto.id == i.item_id,
                Produto.tenant_id == tenant.id
            ).first()
            if produto:
                produto.estoque_atual -= int(i.quantidade)
...
# ---------------------------------------------------------------------
# Logging e bootstrap (super admin)
# ---------------------------------------------------------------------
...
async def startup_event():
    ...
    try:
        ...
        super_admin = db.query(User).filter(User.email == email, User.role == UserRole.SUPER_ADMIN).first()
        if not super_admin:
            super_admin = User(
                id=str(uuid.uuid4()),
                email=email,
...
# Exec local:  uvicorn server:app --reload
